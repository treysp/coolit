...
(c(`Prop. tower` = 1 / params$class_weights$`1`, 
   `Prop. No tower` = 1 / params$class_weights$`0`))
...
message("Initiating model objects.")
...
#### train dense layers ---------------------------------
message("Creating dense training model.")
...
if (params$add_small_final_layer) {
  if (!(is.numeric(params$small_layer_size) && 
        length(params$small_layer_size) == 1)) {
    stop("Small layer size must be a single number.")
  }
...
sink(file.path(params$curr_model_dir, "model-structure.txt"))
print(summary(model))
sink()
...
message("Creating first fine-tune model.")
...
message("Training first fine-tune model:")
before <- Sys.time()
...
message("Creating second fine-tune model.")
...
message("Training second fine-tune model:")
before <- Sys.time()
...
saveRDS(params, file.path(params$curr_model_dir, "run-parameters.RDS"))
